...
import { addHomework, type HwState } from "@/app/teacher/homework/actions";

const initial: HwState = {};
const inputClass =
...
export function TeacherHomeworkForm() {
  const [state, formAction, pending] = useActionState(addHomework, initial);

  return (
    <form action={formAction} className="rounded-3xl bg-surface p-7 shadow-[0_10px_40px_rgba(19,48,41,0.05)]">
      <h2 className="font-display text-xl font-semibold text-ink">Assign homework</h2>
      <div className="mt-5 space-y-4">
        <input name="title" className={inputClass} placeholder="Title (e.g. Maths — Worksheet 3)" />
        <textarea name="description" rows={3} className={`${inputClass} resize-none`} placeholder="Details / instructions" />
        <div>
          <label htmlFor="hw-due" className="font-sans text-sm font-medium text-ink">Due date</label>
          <input id="hw-due" name="due_date" type="date" className={`mt-1.5 ${inputClass}`} />
        </div>
      </div>
